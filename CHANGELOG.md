...
Nothing to do unless a source's `url` ends `.zip` or declares `format: zip` — measured via
`gh api search/code -f q='".zip" org:OregonAI extension:yml'` (and again for
`extension:yaml`, 0 hits): federal-reference's manifest is the only one across the org's
repos with a `.zip` hit, today. `_format_for` had no zip branch, so such a source was hashed
as html/binary over the ARCHIVE bytes, not its content. OLRC's per-title USLM release points
(e.g.
...
platform has one) and hashes the decompressed bytes as whatever format the member's own
filename implies, matching what a corpus that caches the unzipped content computes on its
own ingestion. A zip holding zero or more than one member is refused rather than guessed at,
surfacing as `corpus-detect-changes`'s own `archive_unreadable` outcome — a fact about the
archive's contents, kept out of `failed`/`access-failures.json` because the bytes did
arrive. Bytes that are not a zip at all (an error or login page served at a `.zip` url) fall
back to `format: html` instead of raising, the same inference every other unrecognised
extension already gets. Every existing format hashes byte-for-byte as before
(corpus-toolkit#199).
